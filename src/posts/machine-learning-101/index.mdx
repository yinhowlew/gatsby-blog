...
Machine Learning is getting computers to perform a specific task without explicit instruction. 
...
For example, if we want our program to identify a dog breed from a dog image, we need to feed the program a large quantity of dog images along with the correct labels / answers. 
